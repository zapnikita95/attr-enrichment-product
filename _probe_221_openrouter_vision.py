...
НЕ ВКЛЮЧАЙ В new_attributes (но можешь перечислить в not_useful_from_image):
- любые «без X», «не содержит», MSG-free, Non-GMO, No Preservatives, sugar-free, gluten-free —
  это ломает поиск (запрос «сахар»/«краситель» начнёт матчить товар без сахара)

ИЩИ В ПРИОРИТЕТЕ (только если явно на фото и НЕТ в списках ниже):
- способ обработки на этикетке: копчёный, вяленый, охлаждённый, замороженный, УВТ, hand cooked, фильтрованный
- форма выпуска/нарезка: мельница, нарезка, ломтики, кубики, фарш, филе
- вкус/наполнитель как ПОЗИТИВНЫЙ LABEL (клубника, ваниль, BBQ, brownie) — только если написано и нет в названии
- состав/ингредиенты на этикетке, если их нет в фиде (позитивный состав, не «без…»)
- тип упаковки если НЕ в params: стекло, жесть, tetra, дойпак
- для сыров/колбас: жирность %, выдержка, тип молока — только с этикетки
- визуально очевидное позитивное: цельный плод vs нарезка, цвет мякоти (если помогает поиску)
